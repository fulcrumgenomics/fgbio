...
```mermaid
graph TD;
A["fgbio FastqToBam"]-->B["samtools faidx | bwa mem | fgbio ZipperBam"];
...
## Phase 2a: GroupedBam -> Filtered Consensus, R&D Version


```mermaid
graph TD;
A["fgbio CallMolecularConsensusReads"]-->B["samtools faidx | bwa mem | fgbio ZipperBam"];
B-->C["fgbio FilterConsensusReads | samtools sort"];
```

### Step 2a.1: GroupedBam -> Consensus uBam
...
### Step 2a.2: Consensus uBam -> Consensus Mapped BAM
...
The `-Y` flag is critical at this step otherwise the per-base tags from the consensus caller will get out of sync with the read's bases and quality scores.
...
### Step 2a.3: Consensus Mapped -> Consensus Filtered & Sorted BAM
